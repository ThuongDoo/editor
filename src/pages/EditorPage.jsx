...
export default function EditorPage() {
  ...
  const { schema, loading: schemaLoading, error: schemaError } = useTemplateSchema(
    website?.templateId,
  )
  ...
  if (website.ownerId !== user?.uid) {
    return <p className="form-error">Bạn không có quyền chỉnh sửa website này.</p>
  }

  if (schemaLoading || draft === null) return <p>Đang tải...</p>
  ...
  return (
    <EditorContext.Provider value={{ websiteId }}>
      <main className="editor-page">
        ...
        <SchemaForm sections={schema.sections} value={draft} onChange={setDraft} />

        <div className="editor-actions">
          <button type="button" onClick={handleSave} disabled={!isDirty || saveState === 'saving'}>
            {saveState === 'saving' ? 'Đang lưu...' : 'Lưu'}
          </button>
          {saveState === 'saved' && !isDirty && <span className="save-status">Đã lưu.</span>}
          {saveState === 'error' && <span className="form-error">Lưu thất bại, thử lại.</span>}
        </div>
      </main>
